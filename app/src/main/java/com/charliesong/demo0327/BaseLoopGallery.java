...
public class BaseLoopGallery<T> extends RecyclerView {
    ...
    public void setDatasAndLayoutId(final List<T> datas, int itemLayoutId, BindDataListener bindDataListener) {
        ...
        mAdapter=new BaseRvAdapter<T>() {
            ...
            public void onBindViewHolder(BaseRvHolder holder, int position) {
                ...
                if(getBindDataListener()!=null){
                    getBindDataListener().onBindData(holder,datas.get(position));
                }
            }
            ...
            private int getRealPosition(int position){
                return position % (Math.max(datas.size(),1));
            }
        };
        setAdapter(mAdapter);
    ...
    public interface BindDataListener<T> {
        void onBindData(BaseRvHolder holder, T data);
    }

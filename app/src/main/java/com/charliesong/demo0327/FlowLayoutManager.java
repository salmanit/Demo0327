...
/**
 * Created by charlie.song on 2018/5/3.
 */
...
/*    private class FlowItem {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        System.out.println("line 58=========="+state.toString()+" child count="+getChildCount());
    ...
    private int fill(RecyclerView.Recycler recycler, RecyclerView.State state, int dy) {
        ...
        if (dy >= 0) {
            ...
            }System.out.println("131==========dy="+dy+" minPos="+minPos+" lastVisiPosition=="+mLastVisiPos+" getItemCount="+getItemCount()+" getChildCount=="+getChildCount()+" getHeight=="+getHeight());
            //顺序addChildView
            for (int i = minPos; i <= mLastVisiPos; i++) {
                ...
                if (leftOffset + getDecoratedMeasurementHorizontal(child) <= getHorizontalSpace()) {//当前行还排列的下
                    ...
                    System.out.println("144==========layout="+i);
                    //改变 left  lineHeight
                    ...
                    if (topOffset - dy > getHeight() - getPaddingBottom()) {
                        //越界了 就回收
                        removeAndRecycleView(child, recycler);
                        System.out.println("158==========回收="+i +" topOffset=="+topOffset);
                        mLastVisiPos = i - 1;
                    } else {
                        layoutDecoratedWithMargins(child, leftOffset, topOffset, leftOffset + getDecoratedMeasurementHorizontal(child), topOffset + getDecoratedMeasurementVertical(child));
                        System.out.println("162==========layout="+i);
                        //保存Rect供逆序layout用
                        Rect rect = new Rect(leftOffset, topOffset + mVerticalOffset, leftOffset + getDecoratedMeasurementHorizontal(child), topOffset + getDecoratedMeasurementVertical(child) + mVerticalOffset);
                        mItemRects.put(i, rect);
            ...
            System.out.println("172============mLastVisiPos="+mLastVisiPos+"=====childCount="+getChildCount()+"=childPosition="+getPosition(lastChild));
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        System.out.println("251========mVerticalOffset=="+mVerticalOffset+" realOffset==="+realOffset+" dy=="+dy);
        return realOffset;
    }
